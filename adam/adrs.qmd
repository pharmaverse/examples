...
The Analysis Data Review Set (ADRS) serves as a dataset in clinical trials, particularly in the field of oncology, for monitoring disease progression and therapeutic efficacy. 

ADRS is structured to capture and organize data points pertinent to efficacy endpoints like Progression-Free Survival (PFS). PFS is a vital metric that measures the amount of time a patient lives without their disease worsening after commencing treatment.
...
### Required Packages
The packages below are required to construct an ADRS focusing on PFS parameters. 
```{r, output=FALSE}
library(admiral)
library(admiralonco)
...
`adsl` contains subject-level data and `rs_onco_recist` holds oncology-specific response data. The `rs_onco_recist` dataset is preprocessed, ensuring the data is clean for analysis. 
...
A function is constructed to rank responses from worst to best. The `derive_var_relative_flag` function is then used to flag the worst response for further analysis, considering only valid assessments from the randomization date onwards. 
...
### Handling Death Data
We next need to process the death data. The function `derive_extreme_records` is used to identify the first occurrence of a death event for each patient, ensuring it is recorded and flagged for analysis. 
```{r}
adsldth <- adsl |>
...
additional variables are merged rom `adsl` into the main dataset, enriching it with more subject-level details. 
```{r}
adrs <- adrs |>
  derive_vars_merged(
...
- **PARCAT3**: Denotes the criteria used, "RECIST 1.1", which is a standard for assessing tumor response in trials.
- **ADT**: The actual date of the assessment, showing when each response was evaluated.
- **AVALC**: Represents the assessed value of response, e.g., "SD" (Stable Disease), "NE" (Not Evaluable), "CR" (Complete Response), "PD" (Progressive Disease), reflecting the condition of the disease at the time of assessment.
- **AVAL**: Numeric code of the response, where 1 is best (CR) and 6 is worst (NE), used for statistical analysis.
- **ANL01FL**: A flag ('Y' for yes) indicating that the record is valid for analysis.
- **ANL02FL**: This flag indicates whether the record is before the first occurrence of progressive disease ('Y' for yes, 'NA' for not applicable).
- **RSDTC**: The reported date of the assessment, which should align with ADT but may be less precise (e.g., missing day component).
