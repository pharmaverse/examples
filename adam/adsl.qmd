...
Option 2: We can create the subgroups using the admiral::derive_vars_cat() function available from {admiral} v1.1.2. 
...
We can also assign the Numeric Variables, using the `metatools` package with the `{metacore}` objects
that we created at the very beginning. The function `metatools::create_var_from_codelist()` is used in below exemple.
